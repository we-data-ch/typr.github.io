...
Answers to common questions, cross-linked with the [documentation](intro.md) and [blog](/blog) where relevant.
...
TypR is a **statically typed version of R**, built for package and application developers. You write code in a language that looks almost identical to R, a compiler checks your types, and then it translates everything into ordinary `.R` files.
...
No. TypR is designed to live next to R, not instead of it. There is no new platform, no virtual machine, no special version of R, and no system-level dependency — just one small `typr` binary. Once your code is compiled, TypR disappears: the output is plain R code that runs wherever R runs.

R remains the right tool for research, exploration, and interactive statistics. TypR is for packages and applications that need to survive production. See [Working with R and TypR](reference/r-typr.md) for details.

### 3. Will TypR replace R?

No. TypR was never intended to replace R. Use it only when you need to make R more robust to errors. Use the right tool for the right task.
...
One important nuance: TypR is **not** a strict superset of R. It is a dialect of R with a design flavor borrowed from Rust, Scala, Nim, and Roc.

### 6. Does TypR add system dependencies?

No. The transpiler ships as a single binary. The compiled result has **no dependency on Rust whatsoever**. Rust is only the language the transpiler is written in. You can work from the command line (`typr build`), in a compatible IDE, or in RStudio via the [`typr_runner`](https://www.youtube.com/watch?v=GMo20g__nOc) package. An LSP and a formatter are in development.

### 7. Is TypR just R with Rust syntax?

No. TypR is **gradually typed**, which means plain R code is valid TypR code. You can start with your normal R code and add types incrementally. If you know R, you already know the base syntax of TypR.

Besides Rust, TypR takes inspiration from [Scala](https://www.scala-lang.org/), [Nim](https://nim-lang.org/), and [Roc](https://www.roc-lang.org/).

---

## Getting started

### 8. How do I install TypR?
...
4. Install an IDE if you don't have one:
...
It is also simpler when you need object management. Roughly **70 lines** of typical generated S3 boilerplate (constructors, `missing()` juggling, field-by-field validators) collapse into about **15 lines** of TypR. See [Solving the OOP chaos for R](/blog/typr-oop) for a deeper look.

### 10. Why are there semicolons?

Semicolons are required today to disambiguate statements, but they may become optional in the future. Treat them as practical for the current compiler, not sacred.
...
On top of that, the compiler does **type inference**, filling in types from literals, expressions, and context. You only annotate where clarity or safety really matters. Pick your own point on a freedom-versus-safety spectrum, with R at one extreme and Rust at the other.
...
See the [types reference](reference/types.md) for full details.
...
From then on, the compiler checks that everything using a `Person` type respects the structure. A mistyped field name or a `char` where an `int` belongs fails **at compile time**, before anything runs. Under the hood, this generates standard S3-based R (constructors, validators, classes) that any other R package can consume.

A subtlety worth knowing: `type X <- ...` creates a *distinct* new type, while `type X = ...` creates a mere *alias*, interchangeable with the underlying type. See the [types reference](reference/types.md) for examples.
...
For data frames specifically, **row polymorphism** lets functions declare only the columns they touch:

```julia
let get_age <- fn(df: DataFrame { age: int }): int {
  df$age
};
```

That function accepts every data frame with an integer `age` column, regardless of what other columns it contains. See the [types reference](reference/types.md) for more on structural subtyping.
...
Every TypR function can be called in **three equivalent ways** (uniform function call syntax, inspired by Nim):
...
true.double();   # bool inherits every function written for Viewable
```
...
Reductions come along for the ride (`points |> sum()` uses your type's `+`). Note: with `reduce`, the type system currently cannot infer which operator instance you mean, so you write `` points |> reduce(`+`<Point>) ``.
...
There is strong industry evidence that static typing reduces defect rates. TypeScript's success over JavaScript is one prominent example. The TypR team has used typed languages (Rust, TypeScript, Nim, and others) and wished for similar safety when returning to R. A type system catches bugs before they reach production, where they are harder and more expensive to fix.
